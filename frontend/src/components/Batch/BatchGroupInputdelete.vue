        ...
        <div v-if="!school[fields[1].key]" class="mb-3">
          <ValidationObserver v-slot="{ passes, invalid }">
            <form
              @submit.prevent="
                passes(
                  addValueToTypeInBatchId(jobId, group, school.value, index)
                )
              "
              class="row col-12"
            >
              <div v-for="field in fields" :key="field + index">
                ...
                >
                  ...
                  <b-form-input
                    ...
                    show="false"
                    :disabled="true"
                    v-model="school[field.key]"
                    :ref="field.key + jobId + index"
                    class="col-12"
                  />
                </ValidationProvider>
              </div>
        ...
        <div class="row col-12 mb-2">
          ...
          <div v-if="index != batch.schools.length - 1" class="col-2">
            <b-button
              ...
              @click="
                deleteValueFromTypeInBatchId(jobId, 'schools', school.value)
              "
            >
              Remove
            </b-button>
      ...
      formElements: {
        PSIRUN: {
          group: [
            ...
            { text: "District - N/A", value: "District", disabled: true },
            ...
            if (response.data.minCode) {
              for (let field of this.fields) {
                if (this.$refs[field.key + refValues[0] + refValues[1]]) {
                  this.$refs[
                    field.key + refValues[0] + refValues[1]
                  ][0].placeholder = response.data[field.key];
                }
              }
              return { valid: true };
            } else {
        ...
        SchoolService.getSchoolInfo(value)
          .then((response) => {
            if (response.data.minCode) {
              ...
              this.$refs["address1" + id + valueIndex][0].updateValue(
          ...
          .then((response) => {
            if (response.data.length == 0) {
              ...
              this.$refs["pen" + id + valueIndex][0].updateValue(
                response.data[0].usualFirstName +
                  " " +
                  (response.data[0].usualMiddleNames
                    ? response.data[0].usualMiddleNames + " "
                    : "") +
                  response.data[0].usualLastName
              );
